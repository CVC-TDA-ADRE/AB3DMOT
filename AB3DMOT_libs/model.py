...
class AB3DMOT(object):  # A baseline of 3D multi-object tracking
    ...
    def update(self, dets_all):
        ...
        dets_id = [None for _ in range(len(self.trackers))]
        ...
        for trk in reversed(self.trackers):
            ...
            # TODO: Before 1 was self.max_age
            if (trk.time_since_update < 1) and (
                trk.hits >= self.min_hits or self.frame_count <= self.min_hits
            ):
                ret.append(
                    np.concatenate((d, [trk.id + 1], trk.info)).reshape(1, -1)
                )  # +1 as MOT benchmark requires positive
                dets_id_ret.append(dets_id[i - 1])
            elif dets_id[i - 1] is not None:
                ret.append(np.empty((1, 5)))
                dets_id_ret.append(-1)
            i -= 1
